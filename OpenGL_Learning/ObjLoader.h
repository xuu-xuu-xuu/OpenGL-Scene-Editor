#pragma once
// ObjLoader.h —— OBJ 加载器（v/vn/vt/f/usemtl）
// 输出：交错 位置3+法线3+UV2；并按材质分段记录顶点范围
...
struct ObjMatRange
{
    std::string Material;   // usemtl 材质名
    unsigned int Start = 0; // 起始顶点
    unsigned int Count = 0; // 顶点数
};
...
struct ObjModel
{
    ...
    std::vector<ObjMatRange> Materials;
...
inline bool LoadObjFile(const std::string& path, ObjModel& out)
{
    ...
    struct Face { std::vector<int> p, n, t; std::string mat; };
    ...
    while (std::getline(file, line))
    {
        ...
        if (type == "v")       { float x,y,z; if (ss>>x>>y>>z) positions.push_back(glm::vec3(x,y,z)); }
        else if (type == "vn") { float x,y,z; if (ss>>x>>y>>z) normals.push_back(glm::vec3(x,y,z)); }
        else if (type == "vt") { float u=0,v=0; ss>>u>>v; texcoords.push_back(glm::vec2(u,v)); }
        else if (type == "usemtl") { std::string name; ss >> name; if (!name.empty()) curMat = name; }
        else if (type == "f")
        {
            Face f; f.mat = curMat;
            std::string tok;
            while (ss >> tok)
            {
                ...
                if(s1!=std::string::npos)
                {
                    ...
                    { std::string c=tok.substr(s2+1); if(!c.empty()) ni=std::atoi(c.c_str())-1; }
                }
                if(pi>=0 && pi<(int)positions.size()){ f.p.push_back(pi); f.n.push_back(ni); f.t.push_back(ti); }
            }
    ...
    auto ensureRange = [&](const std::string& mat)
    {
        if(out.Materials.empty() || out.Materials.back().Material!=mat)
        {
            ObjMatRange r;
            r.Material=mat;
            r.Start=(unsigned int)(out.Data.size()/8);
            r.Count=0;
            out.Materials.push_back(r);
        }
    };
    ...
    for(Face& f : faces)
    {
        ...
        for(int n:f.n) if(n<0||n>=(int)normals.size()){ hasNormals=false; break; }
        glm::vec3 flatN(0.0f);
        if(!hasNormals)
        {
            glm::vec3 a=positions[f.p[0]], b=positions[f.p[1]], c=positions[f.p[2]];
            flatN=glm::normalize(glm::cross(b-a,c-a));
        }
        for(int t=1;t+1<count;++t)
        {
            int idx[3]={f.p[0],f.p[t],f.p[t+1]};
            int nidx[3]={-1,-1,-1}, tidx[3]={f.t[0],f.t[t],f.t[t+1]};
            if(hasNormals){ nidx[0]=f.n[0]; nidx[1]=f.n[t]; nidx[2]=f.n[t+1]; }
            for(int k=0;k<3;++k)
            {
                glm::vec3 n=hasNormals?normals[nidx[k]]:flatN;
                pushVertex(idx[k],n,tidx[k]);
                out.Materials.back().Count++;
            }
        }
    }
